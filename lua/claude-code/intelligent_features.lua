...
function M.call_specialized_agent(agent_type, task_type, data, callback)
  ...
  local request_data = {
    messages = {
      {
        role = "system",
        content = string.format("You are the %s agent. Focus on %s. Provide specific, actionable insights.",
          agent.name, table.concat(agent.capabilities, ", "))
      },
      {
        role = "user",
        content = prompt
...
function M.create_agent_prompt(agent_type, task_type, data)
  local base_prompt = string.format("Analyze this %s code from %s:\n\n```%s\n%s\n```\n\n",
    data.file_type, data.filename, data.file_type, data.content)

  local specialized_prompts = {
    validator = {
      quality_analysis = base_prompt .. "Focus on: code quality, best practices, maintainability, and potential improvements.",
      compliance_checking = base_prompt .. "Focus on: coding standards compliance, documentation requirements, and style guidelines."
    },
    security = {
      security_scan = base_prompt .. "Focus on: security vulnerabilities, potential exploits, data exposure risks, and security best practices.",
      vulnerability_detection = base_prompt .. "Focus on: detailed vulnerability analysis with CVE mappings where applicable."
    },
    dise = {
      performance_analysis = base_prompt .. "Focus on: performance bottlenecks, optimization opportunities, algorithmic efficiency, and resource usage.",
      efficiency_analysis = base_prompt .. "Focus on: computational complexity, memory usage, and execution time optimization."
    },
    architect = {
      architecture_review = base_prompt .. "Focus on: architectural patterns, design principles, modularity, and structural improvements.",
      workflow_design = base_prompt .. "Focus on: development workflow optimization and process improvements."
    }
  }

  return specialized_prompts[agent_type] and specialized_prompts[agent_type][task_type] or base_prompt
end
...
function M.get_ai_completion_suggestions(context, callback)
  local request_data = {
    messages = {
      ...
      {
        role = "user",
        content = string.format("Context: %s language, line: '%s', position: %d. Suggest completion.",
          context.file_type, context.line, context.column)
      }
    }
  }
